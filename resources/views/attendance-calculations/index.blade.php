...
<div class="mx-auto max-w-6xl space-y-6">
    ...
    @if(!$selectedPeriod)
        <section><h2 class="text-xl font-semibold">Selecciona un período</h2><div class="mt-4 grid gap-4 md:grid-cols-2">@forelse($periods as $period)@php($summary = $periodSummaries[$period->id])<a href="{{ route('attendance-calculations.index', ['control_period_id' => $period->id]) }}" class="card block transition hover:-translate-y-0.5 hover:border-slate-400 hover:shadow-md"><div class="flex items-center justify-between"><h3 class="text-xl font-bold">{{ $period->name }}</h3><span class="text-2xl" aria-hidden="true">›</span></div>@if(!$summary['calculated'])<p class="mt-4 text-sm font-semibold text-slate-500">Pendiente de cálculo</p>@else<div class="mt-4 grid grid-cols-2 gap-2 text-sm"><p><span class="font-semibold">{{ $summary['compatible'] }}</span> compatibles</p><p><span class="font-semibold">{{ $summary['corrected'] }}</span> corregidas</p><p><span class="font-semibold">{{ $summary['pending'] }}</span> requieren revisión</p><p><span class="font-semibold">{{ $summary['no_marks'] }}</span> sin marcaciones</p></div>@endif</a>@empty<div class="card md:col-span-2 py-12 text-center text-slate-500">Todavía no hay períodos con jornadas interpretadas.</div>@endforelse</div></section>
