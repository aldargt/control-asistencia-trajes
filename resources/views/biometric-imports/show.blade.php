    ...
    <div class="mx-auto max-w-6xl space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                @if($biometricImport->people->flatMap->attendanceInterpretations->isNotEmpty())
                    <a href="{{ route('biometric-imports.interpretation.show', $biometricImport) }}" class="button-secondary">Ver interpretación</a>
                @endif
                <form method="POST" action="{{ route('biometric-imports.interpretation.store', $biometricImport) }}">@csrf<button type="submit" class="button-primary">{{ $biometricImport->people->flatMap->attendanceInterpretations->isNotEmpty() ? 'Reprocesar interpretación' : 'Interpretar marcaciones' }}</button></form>
                <a href="{{ route('biometric-imports.index') }}" class="button-secondary">Volver a importaciones</a>
            </div>
        </div>

        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            @foreach([['Personas detectadas', $biometricImport->people_count], ['Vinculadas', $biometricImport->matched_people_count], ['Con marcaciones', $peopleWithMarksCount], ['Sin marcaciones', $peopleWithoutMarksCount], ['Sin vincular', $biometricImport->unmatched_people_count], ['Advertencias', $biometricImport->warning_count]] as [$label, $value])
